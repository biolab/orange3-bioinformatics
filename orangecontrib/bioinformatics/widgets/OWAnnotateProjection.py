...
from orangecontrib.bioinformatics.annotation.annotate_projection import \
    annotate_projection, cluster_additional_points
...
class Runner:
    ...
    @staticmethod
    def compute_secondary_clusters(embedding: Table, result: Result,
                                   state: TaskState):
    ...
    @classmethod
    def run(cls, data: Table, secondary_data: Table, attr_x: ContinuousVariable,
...
class OWAnnotateProjection(OWDataProjectionWidget, ConcurrentWidgetMixin):
    ...
    class Warning(OWDataProjectionWidget.Warning):
        ...
        missing_compute_value = Msg("{} or {} is missing in Secondary data and"
                                    " cannot be computed from Reference data.")
        missing_tax_id = Msg(f"'{TAX_ID}' is missing in Reference data. "
        ...
        missing_tax_id_genes = Msg(f"'{TAX_ID}' is missing in Genes table. "
                                   f"Try using 'Marker Genes' widget.")
        different_tax_id = Msg(f"Data and Genes appear to describe different "
                               f"organisms (mismatching {TAX_ID}).")
        same_axis_features = Msg(f"Selected features for Axis x and "
                                 f"Axis y should differ.")
    ...
    def __add_annotation_controls(self):
        common_options = dict(
            labelWidth=100, orientation=Qt.Horizontal, sendSelectedValue=True,
    ...
    def can_annotate(self):
        return self.reference_data and self.genes and \
               ...
               ENTREZ_ID in self.genes.domain and \
               CELL_TYPE in self.genes.domain and \
               TAX_ID in self.genes.attributes and \
               self.reference_data.attributes[TAX_ID] == \
               self.genes.attributes[TAX_ID] and \
               self.attr_x is not None and self.attr_y is not None and \
               self.attr_x is not self.attr_y
    ...
    def _run(self):
        ...
        self.start(Runner.run, self.reference_data, self.secondary_data,
                   self.attr_x, self.attr_y, self.genes, self.p_threshold,
    ...
    def check_inputs(self):
        ...
        elif self.reference_data and self.genes and \
                TAX_ID in self.reference_data.attributes and \
                TAX_ID in self.genes.attributes and \
                (self.genes.attributes[TAX_ID] !=
                 self.reference_data.attributes[TAX_ID]):
            self.Warning.different_tax_id()
        elif self.secondary_data and not self.reference_data:
            self.Error.no_reference_data()

        self.data = self.reference_data and self.secondary_data \
            or self.reference_data
        self.init_attr_values_secondary()

    def init_attr_values_secondary(self):
        saved_attrs = {
            "attr_color": self.attr_color, "attr_shape": self.attr_shape,
            "attr_size": self.attr_size, "attr_label": self.attr_label
        }
        domain = self.data.domain if self.data and len(self.data) else None
    ...
    def get_embedding(self):
        def no_embedding_attrs():
            ...
            return self.attr_x.name not in domain and \
                   self.attr_x.compute_value is None or \
                   self.attr_y.name not in domain and \
                   self.attr_y.compute_value is None
    ...
    def get_color_data(self):
        table = self.clusters.secondary_table if self.secondary_data \
    ...
    def send_data(self):
        if self.clusters.secondary_table:
            ...
            self.Outputs.annotated_data.send(
                self._get_annotated_data(data, selection, group_sel,
                                         graph.selection))
        else:
            super().send_data()
    ...
    def _get_projection_data(self):
        ...
        metas = chain(self.scores.table.domain.attributes,
                      self.clusters.table.domain.attributes, domain.metas)
        domain = Domain(domain.attributes, domain.class_vars, metas)
        if self.clusters.secondary_table:
            domain = self.__concatenate_domains(
                domain, self.secondary_data.domain)
    ...
    def __add_data_source_column(self, data):
        source_attr = DiscreteVariable("Data Source",
                                       values=[self.reference_data.name,
...
if __name__ == "__main__":
    ...
    new_dom = Domain(ref_data.domain.attributes, ref_data.domain.class_vars,
    ...
    WidgetPreview(OWAnnotateProjection).run(
        set_data=ref_data,
        set_secondary_data=table_data[1:200:2],
