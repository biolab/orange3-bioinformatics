...
from Orange.widgets.gui import vBox, comboBox, widgetBox
...
class GeneSetSelection(OWComponent, QObject):
    ...
    selection: list
    selection = Setting([('KEGG', 'Pathways')], schema_only=True)

    selected_organism = Setting(11, schema_only=True)
    ...
    def __init__(self, parent_widget, parent_component):
        ...
        # taxonomy
        self.organisms = [name for _, name in taxonomy.COMMON_NAMES]
        self.tax_ids = [ids for ids, _ in taxonomy.COMMON_NAMES]

        box = vBox(self.parent_component, 'Organism', margin=0)
        self.sample_column_combo = comboBox(
            box,
            self,
            'selected_organism',
            items=self.organisms,
            callback=self.selected_organism_changed,
            searchable=True,
        )
        ...
        # custom gene sets
        self.domain_model: DomainModel = DomainModel(
        ...
        self.custom_gs_box = widgetBox(
            self.parent_component, 'Custom Gene Sets', margin=0
        )
    ...
    def num_of_custom_sets(self) -> int:
        custom_gene_sets = self._gene_sets.map_hierarchy_to_sets().get(
    ...
    def _on_item_clicked(self) -> None:
        """
        The problem is that signal 'itemClicked' activates even if
        we don't click on checkbox area. That is why we must check
        if status of checkboxes changed and only then notify widget
        of the change.
        """
    ...
    def _update_tree_widget(self) -> None:
        self._set_tree_widget_model(
            self.hierarchy_tree_widget, self._gene_sets.hierarchies()
        )
        self._set_selection()
    ...
    def _load_gene_sets(self) -> None:
        tax_id = self.selected_organism_to_tax_id()
    ...
    def _set_tree_widget_model(
        self, widget: QTreeWidget, hierarchies: Set[Tuple[str]]
    ) -> None:
        widget.clear()
        ...
        for domain, sub_domains in sorted(
            tree_widget_model.items(), key=lambda x: x[0].lower()
        ):
            parent = QTreeWidgetItem(widget, [domain])
            parent.setFlags(
                parent.flags() & (Qt.ItemIsUserCheckable | Qt.ItemIsEnabled)
            )
    ...
    def _set_selection(self) -> None:
        iterator = QTreeWidgetItemIterator(
            self.hierarchy_tree_widget, flags=QTreeWidgetItemIterator.All
        )
    ...
    def _get_selection(self) -> List[Tuple[str, ...]]:
        ...
        iterator = QTreeWidgetItemIterator(
            self.hierarchy_tree_widget, flags=QTreeWidgetItemIterator.Checked
        )
    ...
    def _extract_gene_sets_from_data(self) -> Optional[List[GeneSet]]:
        ...
        return [
            GeneSet(gs_id=key, hierarchy=domain, name=key, genes=set(value))
    ...
    def set_selected_organism_by_tax_id(self, tax_id):
        name = taxonomy.common_taxid_to_name(tax_id)
        self.selected_organism = self.organisms.index(name)
        self.selected_organism_changed()

    def selected_organism_to_tax_id(self):
        return self.tax_ids[self.selected_organism]
    ...
    def selected_organism_changed(self):
        self.initialize()
